...
public final class Condition implements SearchFilter {
	...
	static Condition newCondition(final String key, final Object value) throws SearchException {
        ...
        if (searchs.length == 0 || searchs.length > 3) {
            throw new SearchException("Condition key format must be : property or op_property");
        }
        ...
        if (searchs.length == 1) {
            ...
            searchProperty = searchs[0];
        } else {
            try {
                operator = SearchOperator.valueOf(searchs[0].toUpperCase());
            	searchProperty = searchs[1];
            } catch (IllegalArgumentException e) {
                throw new InvlidSearchOperatorException(searchProperty, searchs[0]);
            }
        }
    ...
    private Condition(final String searchProperty, final SearchOperator operator, final Object value) {
        ...
        this.value = value;
        this.key = this.operator + separator + this.searchProperty;
    }
