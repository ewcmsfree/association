package com.ewcms.security.resource.service;
...
public class ResourceService extends BaseTreeableService<Resource, Long> {
    ...
    public List<Menu> findMenus(User user) {
        Searchable searchable = Searchable.newSearchable().addSearchFilter("show", SearchOperator.EQ, true).addSort(new Sort(Sort.Direction.DESC, "parentId", "weight"));
    ...
    private static Menu convertToMenu(Resource resource) {
    	...
    	ResourceStyle oneLevelStyle = null;
    	if (parentIdArray.length == 2){
    		oneLevelStyle = resource.getStyle() == null ? ResourceStyle.accordion : resource.getStyle();
    	}
    	
    	if (parentIdArray.length >= 3){
    		oneLevelId = Long.parseLong(parentIdArray[2]);
    	}
    	
    	
        return new Menu(resource.getId(), oneLevelId, oneLevelStyle, resource.getName(), resource.getIcon(), resource.getUrl());
    }

	@Override
	public Resource initRoot() {
	...
	@Override
	public Map<String, Object> treeAttributes(Resource m) {
		Map<String, Object> attributes = Maps.newHashMap();
		attributes.put("identity", m.getIdentity());
		attributes.put("url", m.getUrl());
		attributes.put("show", m.getShow());
		attributes.put("styleInfo", m.getStyleInfo());
		return attributes;
	}
